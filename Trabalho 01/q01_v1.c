...
ArvDisciplinas *buscar_Tema(struct ArvDisciplinas *raiz, const char nome_tema[]);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (auxPodcast != NULL){
                printf("%s\n", auxPodcast->nome);
                for (int k = 0; k < 3; k++) {
                    ArvDisciplinas *auxTema = inserirTema(&(auxPodcast->temas), nome_tema[k]);
            ...
            case 5: {
                ...
                if (plataformaBuscada == NULL){
                    ...
                    if (podcastBuscado == NULL) {
                        ...
                        ArvDisciplinas *temaBuscado = NULL;
                        char buscarTema[20];
...
ArvDisciplinas *buscar_Tema(struct ArvDisciplinas *raiz, const char nome_tema[]){
    struct ArvDisciplinas *encontrou = NULL; // Inicializa a variável encontrou como NULL
